...
void claw_callback( const riki_msgs::Claw& cla) {
	//callback every time the robot's linear velocity is received
	ros::Time current_time = ros::Time::now();

	pressed = cla.pressed;
	rotDirection = cla.rotDirection;
       // ena = cla.ena;
        clawTimeClose = cla.clawTimeClose;
        clawTimeOpen = cla.clawTimeOpen;

	claw_dt = (current_time - last_claw_time).toSec();
	last_claw_time = current_time;
}
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber claw_sub = n.subscribe("claw_info", 50, claw_callback);
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("odom", 50);
  ros::Publisher claw_pub = n.advertise<riki_msgs::Claw>("clawp", 50);
  ...
  while(n.ok()){
    ...
    //get the claw parameters from remote laptop, and set the clawp for sending 
    riki_msgs::Claw clawp;
    clawp.pressed = pressed;
    clawp.rotDirection = rotDirection;
   // clawp.ena = ena;
    clawp.clawTimeClose = clawTimeClose;
    clawp.clawTimeOpen = clawTimeOpen;
